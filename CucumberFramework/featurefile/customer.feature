Feature: validating Supplier module
  ...
  Scenario Outline: As admin user validate add supplier
    ...
    When wait for customer with "xpath" and "(//a[text()='Customers'])[2]"
    When click customer with "xpath" and "(//a[text()='Customers'])[2]"
    ...
    When wait for customernumber with "xpath" and "//input[@id='x_Customer_Name']"
    Then capture data with "name" with "x_Customer_Number"
    ...
    Examples: 
      | customername | adress  | city     | country | cperson | pnumber | mail         | mnumber | note |
